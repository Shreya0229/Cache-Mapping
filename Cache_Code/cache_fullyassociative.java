...
public class cache_fullyassociative {
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println("THE FULLY ASSOCIATIVE CACHE");
		System.out.println();
		...
		if(input.equals("read")) {
			...
			long[] tag_part= new long[16-offset];
			for(int i=0; i<16-offset;i++) {
				tag_part[tag_part.length-1-i]=add_in_binary[add_in_binary.length-offset-1-i];
			}
			...
			for(int i=0; i<cache_line; i++) {
				...
				String tag_str= stringwithbits(tag_array[i], 16-offset);
				...
				for(int j=0; j<block_size; j++) {
					...
					long final_decimal_address= Long.parseLong(tag_str+ datapart_str,2);
		...
		if(input.equals("write")) {
			...
			if(tag_foundat==-1) {
				...
				for(int i=0; i<cache_line; i++) {
					if(tag_array[i]==-5) {
						tag_array[i]=tag_num;
						data_array[i][offset_decimal]= value;
				...
				if(flag==-10) {
					...
					int rand_int= rand.nextInt(cache_line);
					long replaced_tag= tag_array[rand_int];
					
					
					for(int i=0; i<block_size; i++) {
						data_array[rand_int][i]=-5;
					}
					tag_array[rand_int]=tag_num;
					data_array[rand_int][offset_decimal]= value;
